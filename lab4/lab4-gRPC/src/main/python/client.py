...
with Ice.initialize(sys.argv) as communicator:
    ...
    while user_task != 'exit':
        ...
        while user_task == 'login' and user_demand != 'disconnect':
            try:
                user_demand = input("Available commands: balance, deposit, credit, disconnect\n")

                if user_demand == 'balance':
                    LOG.log_msg("Your account balance is " + str(account.getAccountBalance()))
                    ...
                    end_year, end_month, end_day = input("Please provide end year, month and day in format: yyyy-mm-dd\n").split("-")
                    value, currency_type = input("Please provide credit value and currency in format value;currency\n").split(";")

                    start_date = Bank.Date(int(start_day), int(start_month), int(start_year))
                    end_date = Bank.Date(int(end_day), int(end_month), int(end_year))
